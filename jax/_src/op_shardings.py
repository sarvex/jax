...
def op_sharding_to_numpy_indices(
  ...
  if op_sharding.tile_assignment_devices:
    ...
    assert num_devices == math.prod(op_sharding.iota_dimensions)
  ...
  if op_sharding.tile_assignment_devices:
    ...
    dims = reversed([op_sharding.iota_dimensions[d] for d in op_sharding.iota_minor_to_major])
    perm = reversed(op_sharding.iota_minor_to_major)
    tad = np.arange(num_devices).reshape(dims).transpose(perm).reshape(num_devices)
    device_it = iter(tad)
